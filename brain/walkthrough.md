...
* **`SubscriptionComparison.tsx` & `lib/plans.ts` Matrix Alignment**: Adjusted core plan constants to align perfectly with the target capacity math (e.g., Team Mitigations now 30, Agency now 100). Rebuilt the Desktop Spec Sheet matrix to dynamically render a crisp 3-column core grid (Free, Pro, Team). Refactored absent features (e.g. "No" video analysis) to render elegant boolean Rams-style minus icons for a consistent visual presentation.
* **Scale Operations Footer**: Reinstated the dedicated `RSPanel` Agency and Enterprise cards directly beneath the 3-tier matrix.
* **Inline Math Tooltips**: Added inline `ftooltip` info components mapping specific line items inside `PricingCard.tsx`, placing explicit overage / additional mitigation calculations precisely over the scan and mitigation limit text.
