...
- [ ] Create `lib/plans.ts` with all values from this document
- [ ] Update `app/api/stripe/webhook/route.ts` to apply limits on upgrade
- [ ] Update `lib/entitlements.ts` to read from `plans.ts`
- [ ] Add feature flag columns to `tenants` table (migration)
